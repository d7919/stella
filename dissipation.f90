...
!   call calculate_velocity_integrals
  ...
  subroutine init_legendre
    ...
    complex, dimension (:,:,:,:), allocatable :: dum1, dum1a
    ...
    allocate (dum1a(naky,nakx,-nzgrid:nzgrid,ntubes))      
  ...
  call get_fields (gvmu, field(:,:,:,:,1), dum1, dum1a, dist='h') ! note that get_fields sums over species, as required in response matrix
...
!    integer :: imu
  ...
  subroutine init_vpadiff_conserve
    ...
    call get_fields (gvmu, field(:,:,:,:,1), dum1, dum1a, dist='h', skip_fsa=.true.)
